...
- **Scenario probability — target's own history only.** Do NOT build, extend or bind a peer-company calibration cohort to weight a target's scenarios. Peer *panels* are admissible exactly where the quantity being measured is a shared market price — Beta's L1-L4 comparables and PER's peer market residuals — because those cannot be observed from one issuer alone. A scenario probability is not such a quantity: how likely 셀트리온's Bull case is depends on 셀트리온's own economics, not on how 유한양행 evolved. Weight scenarios from the target's own realized driver dispersion, and where that is unavailable, report `UNCALIBRATED` with the reason. The committed steel and REIT cohorts predate this rule and stay for their runs' replay; they are not a precedent to copy.
  - Why this is written down: a cohort was built for 셀트리온 anyway, and it failed in the way this rule predicts — the peer margins (7.0% latest-mean) could not frame a 28.1% biosimilar manufacturer, the Monte Carlo saturated at 99.98% Bull, and the cost was 20+ peer fetches for an answer that carried no discrimination. Cohort mismatch is not a bug to fix with a better cohort; it is what borrowing another company's distribution looks like when it fails loudly. It fails quietly the rest of the time.
