...
현재 Orchestrator는 callback 기반 deterministic 실행 계약을 제공한다. 회사별 workflow는 자신의 active modules와 counterfactual runner adapter를 연결한다. `NOT_MEASURABLE`은 0 영향이 아니며, generic evaluator가 아직 지원하지 않는 모듈은 측정 불가 상태로 보존한다.
