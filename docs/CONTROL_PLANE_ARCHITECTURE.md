...
## 9. Delta revision request orchestration

`src/valuation_engine/revision_orchestration.py` is the maintenance-side controller for user corrections. It does not replace or partially reorder the canonical 33-stage valuation run. It converts a correction into typed clauses and an independent acyclic task graph, while the cyclic Unit Contract consumer graph remains impact-scope evidence only.

Each clause fixes the desired outcome, affected root units and observable acceptance criteria. Each task fixes its owner, clause/unit mapping, read set, write set, dependencies, outputs and validators. Independent tasks share a wave only when they have no dependency and disjoint write sets; an unordered write overlap, dependency cycle, unmapped clause, missing required unit path or stale `base_revision`/`plan_hash` fails closed.

Execution preserves five barriers: evidence before Bridge, Bridge before compilation/model, model before report and generated artifacts, targeted checks before full regression, and full regression before publish/merge. A failed task invalidates only itself and its descendants. Completed independent work is reusable only against the same base revision and plan hash. Report downloads are immutable artifacts whose filename, visible artifact ID and latest manifest bind the as-of date, reference intrinsic value and content identity; the mutable latest alias is never the user-delivery target.
