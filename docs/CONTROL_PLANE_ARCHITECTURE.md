...
The decision-facing report precedes audit material and follows a brokerage-research reading order: 투자 요약 → 가치평가 → 핵심 가정과 위험 → 증권사·시장 비교 → 원문 출처. Visible stage names/statuses are Korean. Raw stage IDs, enums and hash chains are confined to collapsed technical detail and immutable machine artifacts so developer diagnostics cannot displace the investment conclusion.
