...
경로가 당기 지급불능으로 종료되면 계약 스케줄의 기말잔액만 waterfall에 넘기지 않는다. 의무 CAPEX와 최소운영현금을 보전한 뒤 당기 원리금·리스료에 실제로 쓸 수 있는 현금을 선순위부터, 동순위 안에서는 청구액 비례로 배분한다. 못 갚은 당기 이자·원금·리스료는 예정 기말잔액에 다시 더해 구조조정일 청구액으로 넘긴다. 그렇지 않으면 상환 실패를 일으킨 바로 그 채무가 회수청구액에서 사라져 구주주 회수액을 과대평가한다.
...
이 경우 `P`를 출처와 범위가 명시된 둘 이상의 확률벡터 정점으로 구성한 ambiguity set, `M`을 자금조달·만기·회수 가정을 branch 전체에 일관되게 적용한 완결 payoff-model case 집합, `CF_(i,m,t)`를 사건 `i`와 모형 `m`의 `t`기 구주주 지급액으로 둔다.

```text
present_value[p,m,h]
  = sum_i p_i * sum_t CF_(i,m,t) / (1 + h)^t

robust_entry_price
  = min_{p in convex_hull(P), m in M} present_value[p,m,h]
```

기대현재가치는 확률에 선형이므로 확률집합의 볼록껍질에 대한 최소·최대는 선언된 정점에서 발생한다. 따라서 이 산식은 ‘가장 낮은 개별 확률’을 고르는 규칙이 아니다. 각 확률벡터와 **branch 전체에 같은 방식으로 적용된 완결 모형 case**의 조합을 모두 계산하고 그 기대현재가치 하한을 취한다. branch별로 서로 다른 최악 자금조달 가정을 짜깁기하는 것은 금지한다. 조기 구조조정 회수액은 실제 구조조정 시점, 중간 배당은 실제 배당 시점, 생존 시 종료지분가치는 종료시점에서 각각 할인한다.

보고서는 조합별 기대가치 범위와 구속 prior·payoff model을 공개한다. 서로 다른 완결 모형 case가 같은 지급액 벡터로 수렴할 수 있으며, 이는 오류가 아니라 해당 가정 차이가 그 경로의 지급액을 바꾸지 않았다는 결과다. 가정·증거·계산 hash는 각각 보존하되 같은 지급액을 억지로 다르게 만들지 않는다. 하한은 모든 선언 조합에서 요구 기대수익률을 충족시키기 위한 매수 상한이지, 연 12% 달성확률이 75%라는 주장이 아니다. 가중 Q25는 진단값으로만 계산하며, ambiguity set 안에서 Q25를 지지하는 branch가 달라지면 구체 Q25 숫자도 숨긴다. 단일 analyst prior만 있거나 사건별 지급액이 pathwise waterfall로 승인되지 않으면 robust entry도 만들지 않는다.

- 현재 주가는 매수가 산식의 입력이 아니다.
- 매수가와 현재가의 비교는 Intrinsic Freeze 이후에만 수행한다.
- `P(distress)`, P10 손실 및 희석확률을 함께 표시한다.
- 보정 경로분포의 Q25가 0 이하이거나, ambiguity set의 최악 기대현재가치가 0 이하이거나, 기초 지급액 승인에 실패하면 매수가를 만들지 않고 전체 최종보고서 생성을 차단한다. 조건부 숫자를 최종 숫자로 승격하지 않는다. 0은 보고서 단계의 하방가치 절삭이 아니라, 명시적 미래 청산 waterfall 또는 종료시점 유한책임을 통과한 실제 주주지급액에만 허용된다.
