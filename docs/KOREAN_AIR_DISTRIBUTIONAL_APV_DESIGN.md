...
보고서는 조합별 기대가치 범위와 구속 prior·payoff model을 공개한다. 하한은 모든 선언 조합에서 요구 기대수익률을 충족시키기 위한 매수 상한이지, 연 12% 달성확률이 75%라는 주장이 아니다. 가중 Q25는 진단값으로만 계산하며, ambiguity set 안에서 Q25를 지지하는 branch가 달라지면 구체 Q25 숫자도 숨긴다. 단일 analyst prior만 있거나 사건별 지급액이 pathwise waterfall로 승인되지 않으면 robust entry도 만들지 않는다.
