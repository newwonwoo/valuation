...
|---|---|---|
| P1 canonical audit | A qualified fixture produces `AUDITED_FINAL` outside the orchestrator. | The original remediation limited the standalone builder to `DIAGNOSTIC_ONLY`. The dated-payoff follow-up permits `AUDITED_FINAL` only after the builder completes `prism_strict_live_primary/v1` and captures its valuation hash, replay audit, freeze token and execution attestation. |
...
The initial remediation did not integrate the experimental distribution into
canonical orchestration. The dated-payoff completion does: a caller must still
supply `source_bundle_manifest` and `source_bundle_manifest_sha256` in its spec,
pointing to the **original complete** `kr-live-report-bundle/v1` archive, and the
builder then submits typed economic and financing paths to the canonical strict
LIVE_PRIMARY runtime. No local audit flag can substitute for the canonical
replay audit, freeze token or execution attestation.
...
canonical dated-payoff result, route authorization and replay audit have been
hash-frozen. The reader-facing `final_report.md` is the canonical runtime
report; the richer path/case tables remain in `dated_payoff_analysis.md` and
cannot authorize or alter the intrinsic result.
