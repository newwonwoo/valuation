...
지금 돌리려면 정확히 무엇을 하는가"** 만 다룬다. 이 절차 전체가 네 번 실행되어
리포에 박제되어 있고 (회귀: `tests/test_kr_live_runbook.py`), 아래 모든 단계는
그 실행에서 실제로 밟은 것이다. 네 런 모두 완주 회귀이고, 대한제강 런은
...
- `runs/celltrion-068270/` — (주)셀트리온: **KSIC가 제안하고 재무제표가 판정한
  첫 런**. 업종코드 21은 임상단계 바이오텍과 상업단계 의약품 제조사를 함께
  담으므로 두 원형을 모두 노출하고, 부문 자신의 매출·영업이익이
  probabilistic_pipeline 전제를 반증해 capacity_manufacturing만 남긴다.
  IFRS 8 주석이 바이오의약품·케미컬의약품 두 보고부문과 자신의 잔여
  기타부문을 구분하며, 잔여 항목은 대사만 하고 평가하지 않는다
  (`unvalued_residual`). 부문 표는 LLM이 읽고 좌표로 되짚어 검증하는
  `source_bound_extraction`으로 결합된다. 증설 게이트가 실전 개통된 첫 런이기도
  하다 — 제4·5공장은 공시된 계획이지만 부지 통제가 증거로 확인되지 않아
  Core 가치에서 제외된다.
