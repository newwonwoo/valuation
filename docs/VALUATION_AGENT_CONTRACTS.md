...
7. Unverified futures are not automatic 0/100. A success probability or single probability-weighted target requires calibration. An `UNCALIBRATED` analyst prior may support only a source-bound ambiguity set: report its expected-value interval and, when every payoff is independently authorized, a worst-prior expected-return entry ceiling. Never describe that ceiling as a calibrated success probability.
...
- **Scenario probability:** use the target's own realized driver history, not a peer-company distribution. Peer panels remain valid for shared market quantities such as Beta and PER residuals, but another issuer's operating history must not weight the target's Down/Base/Bull cases. If the target lacks enough history, report `UNCALIBRATED` with the reason. A load-bearing analyst prior must be represented by at least two distinct source-bound probability vectors. It may produce an ambiguity-set expected-value interval and worst-prior expected-return reference, but not a single calibrated expected value or a quantile success claim. A small discrete-event Q25 is diagnostic; withhold it when its supporting branch changes inside the ambiguity set.
