...
def login_krx(session: requests.Session) -> dict[str, object]:
    login_id = os.getenv("KRX_ID", "")
    login_pw = os.getenv("KRX_PW", "")
    status: dict[str, object] = {
        "credentials_present": bool(login_id and login_pw),
        "authenticated": False,
    }
    ...
    session.get(login_page, headers={"User-Agent": HEADERS["User-Agent"]}, timeout=30)
    session.get(
        login_jsp,
        headers={"User-Agent": HEADERS["User-Agent"], "Referer": login_page},
        timeout=30,
    )
    ...
    status.update(
        {
            ...
            "authenticated": data.get("_error_code") == "CD001",
...
def fetch_json(session: requests.Session, payload: dict[str, str]) -> dict[str, object]:
    ...
    if response.status_code >= 400:
        raise RuntimeError(
            f"KRX HTTP {response.status_code}; content_type={response.headers.get('content-type')!r}; "
    ...
    try:
        ...
        raise RuntimeError(
            f"KRX returned non-JSON status={response.status_code} body={response.text[:800]!r}"
        ) from exc
...
def main() -> int:
    ...
    authentication = login_krx(session)
    warmup_urls = [
    ...
    try:
        ...
        (OUT / "krx_access_failure.json").write_text(
    ...
    union = frame[["stock_code", "company_name"]].drop_duplicates().sort_values(["stock_code", "company_name"])
    ...
    summary = {
        ...
        "snapshot_member_counts": {snapshot["period_id"]: snapshot["member_count"] for snapshot in snapshots},
        "snapshots": snapshots,
    }
    ...
    manifest = {
        ...
        "constituents_sha256": sha256(OUT / "krx_semiconductor_constituents_2015Q1_2024Q2.csv"),
