...
def validate_pr_snapshot(item: dict, snapshot: dict) -> list[str]:
    ...
    if merged and status != "MERGED_PENDING_ACCEPTANCE":
        errors.append(f"{item_id}: PR #{item.get('github_pr')} is merged but portfolio status is {status}")
    if status == "MERGED_PENDING_ACCEPTANCE" and not merged:
        errors.append(f"{item_id}: portfolio says MERGED_PENDING_ACCEPTANCE but PR #{item.get('github_pr')} is not merged")
