...
_SUFFIX_PATTERNS = (
    ...
    re.compile(r"^(?P<company>.+?)\s*(?:좀\s*)?분석(?:\s*좀)?(?:해\s*줘|해\s*봐|해)?$"),
    re.compile(r"^(?P<company>.+?)\s*(?:좀\s*)?(?:밸류에이션|가치\s*평가)(?:\s*좀)?(?:\s*(?:해\s*줘|해\s*봐|해|시작(?:해\s*줘|해)?))?$"),
    re.compile(r"^(?P<company>.+?)\s*(?:적정주가|적정가치)(?:\s*(?:구해\s*줘|봐\s*줘|분석))?$"),
    re.compile(r"^(?P<company>.+?)\s*프리즘(?:으로)?(?:\s*좀)?(?:\s*(?:분석(?:\s*좀)?(?:해\s*줘)?|시작(?:해\s*줘|해)?|돌려\s*(?:봐|줘)))?$"),
