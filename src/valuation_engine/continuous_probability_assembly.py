...
class ContinuousCalibrationBinding:
    ...
    def validate(self) -> None:
        ...
        minimum_driver_count = 1 if self.self_calibrated else 2
        if (
            len(self.driver_ids) < minimum_driver_count
            or len(set(self.driver_ids)) != len(self.driver_ids)
        ):
            route = "target-history" if self.self_calibrated else "cohort"
            raise ContinuousCalibrationError(
                "continuous calibration binding requires at least "
                f"{minimum_driver_count} distinct driver(s) for the {route} route"
            )
