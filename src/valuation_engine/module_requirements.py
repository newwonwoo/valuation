...
def build_module_requirement_plan(
    ...
    *,
    ...
    for risk in special_risks:
        scanner_alias = risk_aliases.get(risk)
        if not scanner_alias:
            continue
        scanner_id = str(scanner_alias)
        existing = scanner_accumulators.get(scanner_id)
        if existing is None:
            existing = _ScannerAccumulator(scanner_id)
            scanner_accumulators[scanner_id] = existing
            interaction_group = f"risk:{risk}"
        else:
            # The archetype contract owns the economic interaction group. A sector-risk
            # alias adds provenance/activation context but must not silently reclassify it.
