...
def evidence_records():
    records = []
    for index, metric in enumerate(REQUIRED_OPERATING, start=1):
        records.append(
            EvidenceRecord(
...
def red_team_officer(context, hypotheses):
    return RedTeamProposal(
        issues=(),
        counter_thesis="The cycle may normalize faster, but the scenario range already preserves that uncertainty without market anchoring.",
    )
