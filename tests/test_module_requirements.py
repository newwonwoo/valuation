...
def test_sector_risk_alias_enriches_existing_scanner_without_reclassifying_group():
    plan = build_module_requirement_plan_from_repo(
        _profile(
    ...
    qualification = next(scanner for scanner in plan.scanners if scanner.scanner_id == "QUALIFICATION")
    assert qualification.interaction_group == "capacity_execution"
    assert "sector_risk:qualification" in qualification.origins
...
def test_every_sector_adapter_default_route_compiles_to_a_nonempty_plan():
    data = yaml.safe_load((REPO_ROOT / "config/sector_adapter_registry.yaml").read_text(encoding="utf-8"))
    adapters = data["adapters"]
    assert adapters

    for adapter_id, adapter in adapters.items():
        default_values = tuple(adapter.get("default_archetypes", ()))
        assert default_values, adapter_id
        archetypes = tuple(EconomicArchetype(value) for value in default_values)
        plan = build_module_requirement_plan_from_repo(
            _profile(adapter_id, archetypes, segment_id=adapter_id),
            repo_root=REPO_ROOT,
        )
        assert plan.required_evidence, adapter_id
        assert plan.allowed_valuation_methods, adapter_id
        assert plan.scanners, adapter_id
        assert len(plan.mandatory_scanner_ids) >= 1, adapter_id
