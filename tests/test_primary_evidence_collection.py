...
def test_source_snapshot_hash_is_order_independent_across_collectors():
    ...
    one = collect_primary_evidence(
        target_id="T",
        required_metrics=("revenue",),
        collectors=(a, b),
    )
    two = collect_primary_evidence(
        target_id="T",
        required_metrics=("revenue",),
        collectors=(b, a),
    )
...
def test_non_primary_market_layer_is_rejected():
    bad = collector(
        ...
        evidence(
            ...
            10000,
            "KRW",
            layer=EvidenceSourceLayer.MARKET_COMPARISON,
        ),
    )
    try:
        collect_primary_evidence(
...
def test_target_mismatch_is_rejected():
    bad = collector(
        "DART",
        evidence("E1", "revenue", 100, "KRW_billion", target="OTHER"),
    )
    try:
        collect_primary_evidence(
            target_id="T",
...
def test_malformed_checked_at_is_rejected():
    malformed = static_evidence_collector(
        source_id="DART",
        checked_at="not-a-date",
        records=(evidence("E1", "revenue", 100, "KRW_billion"),),
        source_fingerprint="FP:DART",
    )
    try:
        collect_primary_evidence(
            target_id="T",
            required_metrics=("revenue",),
            collectors=(malformed,),
        )
    except ValueError as exc:
        assert "checked_at must be an ISO date/timestamp" in str(exc)
    else:
        raise AssertionError("malformed checked_at must fail closed")


def test_timezone_naive_checked_at_timestamp_is_rejected():
    naive = static_evidence_collector(
        source_id="DART",
    ...
    try:
        ...
        assert "checked_at timestamp must be timezone-aware" in str(exc)
    else:
        raise AssertionError("timezone-naive checked_at must fail closed")
...
def test_control_plane_collection_and_ledger_stages_pass():
        ...
        adapters={
            "PRIMARY_EVIDENCE_COLLECTION": primary_evidence_collection_adapter(
                collectors=(
                    collector(
                        "DART",
                        evidence("E1", "revenue", 100, "KRW_billion"),
                    ),
                ),
            ),
    ...
    assert [item.status for item in result.stage_traces] == [
        StageStatus.PASS,
        StageStatus.PASS,
    ]
...
def test_missing_required_metric_enters_recovery_not_silent_pass():
    result = run_controlled_workflow(
        ...
        initial_data={
            "target_id": "T",
            "required_evidence": ("revenue", "margin"),
        },
    )
    assert result.blocked_reasons
