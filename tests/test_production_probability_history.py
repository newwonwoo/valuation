...
import valuation_engine.production_probability_history as production
...
def _set_clock(monkeypatch, value: datetime) -> None:
    monkeypatch.setattr(production, "_utc_now", lambda: value)
...
def _append_forecast(
    ...
    monkeypatch,
    *,
    forecast_id: str = "F-001",
):
    _set_clock(monkeypatch, FORECAST_RECORDED)
...
def test_public_append_api_has_no_recorded_at_override():
    assert "recorded_at" not in inspect.signature(append_production_forecast).parameters
    assert "recorded_at" not in inspect.signature(append_production_outcome).parameters


def test_initialize_append_resolve_and_export_round_trip(tmp_path, monkeypatch):
    path = _history(tmp_path)
    initialized = initialize_production_history(path)
    assert initialized["status"] == "INITIALIZED"
    assert initialized["event_count"] == 0

    forecast_result = _append_forecast(path, monkeypatch)
    ...
    _set_clock(monkeypatch, OUTCOME_RECORDED)
...
def test_first_seen_is_writer_controlled_and_cannot_pollute_earlier_replay(
    tmp_path,
    monkeypatch,
):
    path = _history(tmp_path)
    _append_forecast(path, monkeypatch)
    snapshot = load_production_history(path)
...
def test_duplicate_forecast_rejection_leaves_journal_byte_identical(
    tmp_path,
    monkeypatch,
):
    path = _history(tmp_path)
...
def test_binary_outcome_without_primary_evidence_is_not_appended(
    tmp_path,
    monkeypatch,
):
    path = _history(tmp_path)
...
def test_future_observation_cannot_be_first_seen_earlier(tmp_path, monkeypatch):
    path = _history(tmp_path)
    _append_forecast(path, monkeypatch)
    _set_clock(monkeypatch, OUTCOME_RECORDED)
...
def test_tampered_payload_fails_event_hash_validation(tmp_path, monkeypatch):
    path = _history(tmp_path)
    _append_forecast(path, monkeypatch)
...
def test_broken_previous_hash_fails_chain_validation(tmp_path, monkeypatch):
    path = _history(tmp_path)
    _append_forecast(path, monkeypatch)
    _set_clock(monkeypatch, OUTCOME_RECORDED)
    append_production_outcome(
...
def test_export_cannot_overwrite_the_append_only_journal(tmp_path, monkeypatch):
    path = _history(tmp_path)
    _append_forecast(path, monkeypatch)
    with pytest.raises(
        ProductionProbabilityHistoryError,
...
def test_permissive_history_mode_fails_validation(tmp_path, monkeypatch):
    path = _history(tmp_path)
    _append_forecast(path, monkeypatch)
    os.chmod(path, 0o644)
    with pytest.raises(
...
def test_cli_init_validate_and_summary(tmp_path, monkeypatch, capsys):
    path = _history(tmp_path)
