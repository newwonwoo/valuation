...
def _portfolio(*items):
    return {"legacy_dependency_refs": ["LEGACY-DONE"], "work_items": list(items)}


def _item(item_id, status="READY", dependencies=None, github_pr=None, owner="owner"):
    item = {"id": item_id, "status": status, "owner": owner, "dependencies": list(dependencies or [])}
...
def test_local_validation_accepts_known_legacy_dependency_reference():
    assert validate_local(_portfolio(_item("NEXT", dependencies=["LEGACY-DONE"]))) == []


def test_local_validation_rejects_unknown_dependency_and_cycle():
    errors = validate_local(_portfolio(_item("A", dependencies=["B"]), _item("B", dependencies=["A", "MISSING"])))
...
def test_local_validation_rejects_multiple_active_items_for_same_owner():
    errors = validate_local(_portfolio(_item("A", status="ACTIVE", owner="same"), _item("B", status="ACTIVE", owner="same")))
...
def test_github_snapshot_rejects_merged_pr_left_active():
    ...
    assert validate_pr_snapshot(item, {"state": "closed", "merged_at": "2026-08-25T12:00:00Z"}) == [
        "A: PR #10 is merged but portfolio status is ACTIVE"
    ]
...
def test_github_snapshot_requires_active_pr_to_remain_open():
    item = _item("A", status="ACTIVE", github_pr=10)
    assert validate_pr_snapshot(item, {"state": "closed", "merged_at": None}) == ["A: ACTIVE PR #10 is not open"]
