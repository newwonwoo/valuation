...
def test_celltrions_own_history_is_refused_and_says_why():
    """The committed run's UNCALIBRATED verdict, checkable rather than told.

    The peer-cohort refusal this replaced was a story: it turned on a claim
    about pharmaceutical margins that no one could re-derive from the run. This
    one is a fact about 셀트리온 alone. The 2023-12-28 absorption of
    셀트리온헬스케어 leaves FY2024 and FY2025 on one basis, and revenue growth
    needs its prior year on that same basis, so exactly one period survives —
    against the five transitions a dispersion requires.
    """
    from valuation_engine.self_calibration_factory import (
        MIN_OWN_TRANSITIONS,
    ...
    assert len(observations) < MIN_OWN_TRANSITIONS + 1

    with pytest.raises(CalibrationFactoryError, match="own transitions") as excinfo:
        build_self_calibration_artifact(
            ...
            path_length=YEARS,
            target_ticker=TICKER,
            series_basis=str(payload["series_basis"]),
        )
    assert "0 own transitions across 1 filed periods" in str(excinfo.value)
